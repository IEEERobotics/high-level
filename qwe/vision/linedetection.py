...
class LineDetector(FrameProcessor):
  ...
  State = Enum(['INIT', 'LIVE', 'DONE', 'FAILED'])
  MIN_GOOD_FRAMES = 5
  MAX_BAD_FRAMES = 15
  ...
  def initialize(self, imageIn, timeNow):
    ...
    self.state = LineDetector.State.LIVE
    self.active = True
  ...
  def process(self, imageIn, timeNow):
    ...
    if self.gui: cv2.imshow("Corner match", matchCorner)
    ...
    if self.gui: cv2.circle(self.imageOut, bestLocCorrected, 25, (0, 255, 255), 2)
    ...
    bestLoc = minLoc if methodMatchCorner == cv2.cv.CV_TM_SQDIFF_NORMED else maxLoc
    bestLocCorrected = tuple(np.add(bestLoc, self.templateCornerOffset))
    if self.gui: cv2.circle(self.imageOut, bestLocCorrected, 25, (255, 255, 0), 2)
