...
class BlobTracker(DependentFrameProcessor):
  ...
  def process(self, imageIn, timeNow):
    ...
    # * Draw, report blobs
    if len(self.blobs) > 0:
      self.logd("process", "[%.2f] %d blob(s): %s" % (timeNow, len(self.blobs), ", ".join([str(blob) for blob in self.blobs])))  # preceding ~ indicates blob is not active (i.e. not seen in this frame)
      if self.gui:
        for blob in self.blobs:
          #print blob.detail()  # [debug]
          blob.draw(self.imageOut, self.debug)  # if debug, draw rotated rects as well (requires some additional computation and drawing steps)
      
      centerBlob = self.getNearestBlob(minArea=self.maxBlobArea / 2)  # [debug] by default finds near center of image
      if centerBlob is not None: self.logi("process", "[%.2f] Center blob: %s" % (timeNow, centerBlob))  # [debug]
      if self.gui and centerBlob is not None: cv2.putText(self.imageOut, centerBlob.tag, centerBlob.center, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)  # [debug]
