...
class MapClass:
	...
	def fillLoc(self, waypoints, key, prop):	#fill a location in a layer with a value (both given by prop dict).  The location to be filled is identified by key (key comes from waypoints['key'], i.e. key could be "L01")
		...
		if key[0] is "L": 			#key is a land location
			...
			y1 = int(y - res * mvars.map_real_vars['offset'] - res * mvars.map_real_vars['whiteLine'])
			x2 = int(x - 1.5/2*res)
			y2 = int(y1 - 4*res +1)		#land blocks are 4 in long
		elif key[0] is "A":			#Air
			x1 = int(x - res * mvars.map_real_vars['offset']- res * mvars.map_real_vars['whiteLine'])
